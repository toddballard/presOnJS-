...
ColReorder.prototype = {
	...
	{
                console.log('column picked up '+$(nTh).width());
		var
			that = this,
		...
		$(document).bind( 'mousemove.ColReorder', function (e) {
			that._fnMouseMove.call( that, e ,nTh);
		} );
		
		$(document).bind( 'mouseup.ColReorder', function (e) {
			that._fnMouseUp.call( that, e , nTh);
		} );
	},
	 ...
	 *  @private 
	 */
	"_fnCreateDragNode": function (nTh)
	{
		var that = this;
		...
		this.dom.pointer.style.position = "absolute";
                $(this.dom.pointer).html('<div>'+$(nTh).html()+'</div>');
                
                // Take this code out
                this.dom.pointer.style.width = ($(nTh).width()-30)+"px";
                console.log("set width to: "+$(nTh).width()+"px");
                // ----------------------------------
